...
type MessageSource<T extends MessageParameters> = {
	[key in keyof T]: T[key] extends Record<string, any>
		? Exclude<T[key][keyof T[key]], Record<string, any>> extends never
			? MessageSource<T[key]>
			: Exclude<T[key][keyof T[key]], Record<string, any>> extends
					| boolean
					| Date
					| string
					| number
...
type FlattenRecord<T> = T extends string
	? []
	: {
			[K in Extract<keyof T, string>]: Exclude<
				T[K][keyof T[K]],
				Record<string, any>
			> extends never
				? [K, ...FlattenRecord<T[K]>]
				: Exclude<T[K][keyof T[K]], Record<string, any>> extends
						| boolean
						| Date
						| string
						| number
...
type FlattenParameters<T extends MessageParameters> = UnionToIntersection<
	JoinParameters<FlattenRecord<T>, ".">
>;
...
type FlattenMessageSource<T extends MessageSource<any>> = Record<
	DotNotation<TuplefyKeys<T>, ".">,
	string
>;
...
type MessageBundle<T extends MessageParameters> = {
	getMessage: <K extends keyof FlattenParameters<T>>(
		key: K,
		...parameters: FlattenParameters<T>[K] extends void
			? [undefined?]
			: [FlattenParameters<T>[K]]
	) => string;
	updateBundle: (
		newMessageSrc:
	...
	useBundle: <K extends keyof FlattenParameters<T>>(
		...
		key: K;
		parameters?: FlattenParameters<T>[K];
	};
};
...
): Record<string | number | symbol, (...args: any[]) => string> =>
	Object.entries(object).reduce(
		(acc, [key, item]) => ({
			...acc,
			[key]: _.template(item),
